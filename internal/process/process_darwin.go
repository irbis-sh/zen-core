...
// PROC_PATH_MAXSIZE sets the buffer length for proc_name. It's not defined
// in libproc.h, and various codebases use values from 64 to 4096, but 1024 is likely ok.
const PROC_PATH_MAXSIZE = 1024
...
func FindBySourcePort(port uint16) (Process, error) {
	...
	case ret < 0:
		return Process{}, fmt.Errorf("find pid for port %d: %s", port, C.GoString(C.strerror(C.int(-ret))))
	}
	...
	if ret < 0 {
		return Process{}, fmt.Errorf("find path for pid %d: %s", pid, C.GoString(C.strerror(C.int(-ret))))
	}
	p.DiskPath = C.GoString(&pathBuf[0])

	var nameBuf [PROC_PATH_MAXSIZE]C.char
	ret = C.find_process_name_by_pid(pid, &nameBuf[0], C.size_t(len(nameBuf)))
	if ret < 0 {
		return Process{}, fmt.Errorf("find name for pid %d: %s", pid, C.GoString(C.strerror(C.int(-ret))))
	}
